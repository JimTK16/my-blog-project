...
/* ──────────────────────────────────────────────────────────────────────────
   PostCard — A compact, equal-weight card used in the home feed grid.
    ...
    <Link href={`/blog/${post.slug}`} className='group block h-full'>
      <article
        ...
        className='flex h-full flex-col overflow-hidden rounded-lg border border-border
                   bg-surface p-5 shadow-card hover:-translate-y-1 hover:shadow-elevated
                   transition-all duration-200'
        aria-label={post.title}
      >
        {/* ── Featured image ── */}
        <div className='mb-4 aspect-[16/9] w-full overflow-hidden rounded-md bg-surface-tinted'>
          {post.card_image_url ? (
            <img
        ...
        <div className='flex grow flex-col'>
          ...
          {/* Title — Lora serif heading, reduced to text-lg for compactness */}
          <h2
            className='mb-2 font-heading text-lg font-bold tracking-tight
